...
// World ID imports modified to use standard SIWE flow
import './App.css';
...
function App() {
  ...
  const WORLD_APP_ID = process.env.REACT_APP_WORLD_ID_APP_ID || 'app_2c25889e31357b3124c625f0f5db188f';
  const WORLD_CLIENT_SECRET = process.env.REACT_APP_WORLD_ID_CLIENT_SECRET || 'sk_5dd8d8e94144d0196f0546317991c9eb6e98b63dbfa1ae48';
  ...
  const onWorldIDSuccess = (result) => {
    ...
    // Legacy World ID verification handler (not used in wallet auth)
  };

  // New World ID wallet authentication handler
  const handleWorldIDWalletAuth = useCallback(async () => {
    setLoading(true);
    if (!MiniKit.isInstalled()) {
      showErrorMessage('World App não está instalado ou MiniKit não foi inicializado.');
      setLoading(false);
      return;
    }

    try {
      // TODO: Implement backend endpoint to generate a nonce
      const nonceResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/worldid/nonce`, { method: 'POST' });
      if (!nonceResponse.ok) {
        throw new Error('Falha ao obter nonce do backend.');
      }
      const { nonce } = await nonceResponse.json();

      const siwePayload = await MiniKit.commandsAsync.walletAuth({
        nonce: nonce,
        statement: 'Faça login no BanKa com sua carteira via World ID.',
        // expirationTime: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // Optional: 7 days
        // notBefore: new Date(Date.now() - 1 * 60 * 60 * 1000), // Optional: 1 hour ago
      });

      if (siwePayload.status === 'error') {
        showErrorMessage(`Erro de autenticação World ID: ${siwePayload.error?.message || 'Erro desconhecido'}`);
        setLoading(false);
        return;
      }

      // TODO: Implement backend endpoint to verify SIWE payload and log in user
      const verifyResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/worldid/verify-siwe`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          nonce: nonce,
          message: siwePayload.message,
          signature: siwePayload.signature,
          address: siwePayload.address,
          // chainId: siwePayload.chainId, // if needed by backend for verification
        }),
      });

      if (!verifyResponse.ok) {
        const errorData = await verifyResponse.json();
        throw new Error(errorData.detail || 'Falha ao verificar autenticação com o backend.');
      }
      ...
      if (loginData.token && loginData.user) {
        ...
        setIsLoggedIn(true);
        setAuthComponent(null); // Close auth modal
        showSuccessMessage('Login com World ID Wallet realizado com sucesso!');
        // Fetch user assets or perform other post-login actions
        if (loginData.user.wallet_address) {
            fetchUserAssets(loginData.user.wallet_address);
        }
      } else {
        throw new Error('Resposta de login inválida do backend.');
      }

    } catch (error) {
      console.error('Erro no handleWorldIDWalletAuth:', error);
      showErrorMessage(`Erro ao conectar com World ID Wallet: ${error.message}`);
    }
    setLoading(false);
  }, []);
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <button
            onClick={handleWorldIDWalletAuth}
            disabled={loading}
          ...
          >
            <span className="text-xl">🌍</span>
            <span>{isLogin ? 'Login' : 'Registrar'} com World ID Wallet</span>
          </button>
        </div>
