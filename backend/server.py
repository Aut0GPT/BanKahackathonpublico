...
FRONTEND_URL = os.environ.get('FRONTEND_URL', 'http://localhost:3000') # For SIWE domain verification
...
class WorldIDSIWEPayload(BaseModel):
    message: str
    signature: str
    nonce: str # The nonce originally sent to the client
    address: str # Wallet address from client, to cross-check with message
...
# World ID SIWE Nonce Generation
@app.post("/api/worldid/nonce")
async def generate_worldid_nonce():
    """Generate a secure nonce for World ID SIWE process."""
    nonce = secrets.token_hex(32) # Generate a 32-byte random hex string
    # Optionally, store the nonce with an expiration time if needed for later verification steps
    # For now, just return it directly.
    return {"nonce": nonce}

@app.post("/api/worldid/verify-siwe")
async def verify_worldid_siwe(payload: WorldIDSIWEPayload):
    """Verify a SIWE message and authenticate the user."""
    try:
        siwe_message = SiweMessage(message=payload.message)

        # Determine the expected domain from FRONTEND_URL
        # FRONTEND_URL might be http://localhost:3000, we need 'localhost:3000'
        expected_domain = FRONTEND_URL.split('//')[-1] 

        # Verify the signature and message fields
        # The verify method checks domain, nonce, expiration, not_before, etc.
        siwe_message.verify(
            payload.signature,
            domain_binding=expected_domain,
            nonce=payload.nonce,
            timestamp=datetime.now(timezone.utc)
        )
        
        # Ensure the address in the message matches the one provided by MiniKit
        if siwe_message.address.lower() != payload.address.lower():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Address mismatch between message and payload.")
        ...
        if not user:
            ...
            # Generate a secure, unique, and non-guessable password for internal use
            # This password is not meant for the user to log in with directly via a password field
            internal_password = secrets.token_urlsafe(32) 
            hashed_internal_password = hash_password(internal_password)

            new_user_data = {
                "id": user_id,
                "name": f"World ID User {wallet_address[:6]}...{wallet_address[-4:]}",
                "email": f"worldid_{wallet_address}@banka.com", # Unique email based on wallet address
                "hashed_password": hashed_internal_password,
        ...
        # Prepare user data for JWT and response (excluding sensitive fields like hashed_password)
        user_data_for_token = {
            "id": user["id"],
            "email": user["email"],
        ...
        return {"token": token, "user": user_data_for_frontend}

    except HTTPException as e:
        raise e # Re-raise HTTPExceptions directly
    except Exception as e:
        # For other exceptions (e.g., SiweError subclasses like InvalidSignature, ExpiredMessage)
        # log the error for debugging
        print(f"SIWE Verification Error: {e}")
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"SIWE verification failed: {str(e)}")
